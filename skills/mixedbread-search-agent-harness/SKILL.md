...
Reference harness, the one the model was trained in: https://github.com/mixedbread-ai/toast-harness
...
- In every mode, tell the model to base the answer only on retrieved evidence and to say so when it is insufficient. The exact prompt text per mode is in [reference-harness.md](references/reference-harness.md).
...
Bootstrap is optional: the reference harness fetches metadata facets and one seed search concurrently before round 1 and inserts both as labeled ordinary context (facets first), never as synthetic assistant tool calls or `role="tool"` messages — those roles mean the model spent a turn. The first model generation is round 1 either way. Skip bootstrap when the backend has no facets to offer.
...
| Payload allocation | Bound the round, and keep one large result from starving its siblings — a floor per item. The reference harness water-fills; any fair split works |
...
- [reference-harness.md](references/reference-harness.md) — read when choosing budgets or prompts, or when a run burns rounds, clips evidence, or ignores the context limit. The full system prompt and numbers of Mixedbread's production harness.
